...
class UnicycleModel(Model):
    ...
    def step(self, u: np.array):
        ...
        x, y, th = np.asarray(self._state).flatten()

        x += v * np.cos(th) * self._dt
        y += v * np.sin(th) * self._dt
        th += omega * self._dt
        th = np.arctan2(np.sin(th), np.cos(th))

        self._state = np.array([x, y, th])
        return self._state
...
class AckermanModel(Model):
    ...
    def step(self, u: np.array):
        ...
        x += v * np.cos(th) * self._dt
        y += v * np.sin(th) * self._dt
        th += (v / self.l) * np.tan(delta) * self._dt
        th = np.arctan2(np.sin(th), np.cos(th))
...
class Rectangle(Obstacle):
    ...
    def _calc_points(self, margin=0.0):
        half_width = (self._width + margin) / 2
        half_height = (self._height + margin) / 2
        points = np.zeros((4, 2))
        points[0, 0] = (
            self._center[0] + half_width * np.cos(self._orientationRad) - half_height * np.sin(self._orientationRad)
        )
        points[0, 1] = (
            self._center[1] + half_width * np.sin(self._orientationRad) + half_height * np.cos(self._orientationRad)
        )
        points[1, 0] = (
            self._center[0] - half_width * np.cos(self._orientationRad) - half_height * np.sin(self._orientationRad)
        )
    ...
    def _inside(self, point: np.array, margin=0.0):
        distance = self.distance(point)
        ...
        # or other technique
        width = self._width + margin
        height = self._height + margin

        translated_point = point[:2] - self._center

        cos_angle = np.cos(-self._orientationRad)
        sin_angle = np.sin(-self._orientationRad)

        rotated_point = np.array(
            [
                cos_angle * translated_point[0] - sin_angle * translated_point[1],
                sin_angle * translated_point[0] + cos_angle * translated_point[1],
            ]
        )
...
class MPC:
    ...
    def run(self, start: np.array, goal: np.array, obstacles=[], maxiter=100):
        ...
        stats = {"step": [0], "robot": [start], "goal": goal, "dt": self.dt, "distance": [0], "angle": [0], "cost": [0]}
        ...
        for i in range(1, maxiter):
            ...
            result = minimize(
                self.cost, u, method="SLSQP", constraints=[], options={"ftol": 1e-3, "disp": False}, bounds=bounds
            )
            # TODO
            ...
            self.model.state = self.model_state.copy()
            ...
            path.append(self.model_state.copy())
            ...
            print(
                f"Step {i:05d}, cost {result.fun:.2f}\n"
    ...
    def cost(self, u: np.array) -> float:
        ...
        for i in range(0, steps):
            ...
            distance = np.linalg.norm(state[:2] - self._goal[:2])
            ang = np.arctan2(np.sin(state[2] - self._goal[2]), np.cos(state[2] - self._goal[2]))  # wrapped
            abs_ang = abs(ang)

            # TODO: 3. using the distance to goal
            # calculate cost and add it to overall 'cost'
            cost += 1.0 * distance + 0.1 * abs_ang
    ...
    def plot(self, path: list, goal: np.array, dt: float, animationFile=""):
        ...
        ax.quiver(
            goal[0], goal[1], np.cos(goal[2]) * 0.5, np.sin(goal[2]) * 0.5, angles="xy", scale=1, scale_units="xy"
        )
        ax.grid("both")
        ...
        def init():
            ...
            for obstacle in self._obstacles:
                if obstacle.plotType() == "circle":
                    ax.add_patch(patches.Circle(obstacle.center, obstacle.radius + obstacle.safe_margin, color="pink"))
                    ax.add_patch(patches.Circle(obstacle.center, obstacle.radius, color="red"))
                elif obstacle.plotType() == "rectangle":
                    ax.add_patch(
                        patches.Rectangle(
                            obstacle.get_bottom_left_margin(),
                            obstacle.width + obstacle.safe_margin,
                            obstacle.height + obstacle.safe_margin,
                            angle=obstacle.orientationDeg,
                            color="pink",
                        )
                    )
                    ax.add_patch(
                        patches.Rectangle(
                            obstacle.get_bottom_left(),
                            obstacle.width,
                            obstacle.height,
                            angle=obstacle.orientationDeg,
                            color="red",
